...
public abstract class Distribution {
    ...
    public double getMaxDensity() {
        double max = 0, d;
        for (int i = 0; i < domain.getPartitionSize(); i++) {
            d = getDensity(domain.getValore(i));
    ...
    public double getMean() {
        ...
        if (type == DISCRETE) {
            ...
            w = domain.getWidthPartition();
        }
        for (int i = 0; i < domain.getPartitionSize(); i++) {
            x = domain.getValore(i);
    ...
    public double getCDF(double x) {
        ...
        if (j < 0) {
            return 0;
        } else if (j >= domain.getPartitionSize()) {
            return 1;
        } else {
            for (int i = 0; i <= j; i++) {
                sum = sum + getDensity(domain.getValore(i)) * w;
            }
            if (type == CONTINUOUS) {
                y = domain.getValore(j) - 0.5 * w;
    ...
    public double getQuantile(double p) {
        ...
        if (p <= 0) {
            ...
            int n = domain.getPartitionSize(), i = 0;
            x = domain.getValore(i);
            sum = getDensity(x) * w;
            while ((sum < p) & (i < n)) {
                i++;

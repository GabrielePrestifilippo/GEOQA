 ...
 * matrices, permutation ArrayLists, and the like, produce results in five
 ...
 * ------------------------ */
    ...
    public double mean() {
        double media = 0;
        for (int cont = 0; cont < this.getRowDimension(); cont++) {
            media += (this.get(cont, 0));
        }
        media = media / (this.getRowDimension());
        return media;
    }

    public double stimaVarianza(int numParametri) {
        Matrix var = this.transpose().times(this);
        return var.get(0) / (this.getRowDimension() - numParametri);
    }

    public double varianza(double media) {
        double var = 0;
        if (this.getColumnDimension() == 1 && this.getRowDimension() > 1) {
            for (int cont = 0; cont < this.getRowDimension(); cont++) {
                var += Math.pow((this.get(cont) - media), 2);
            }
            var = var / (this.getRowDimension() - 1);
        }
        return var;
    }
